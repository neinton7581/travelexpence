...
- 底下的「全部套用」是快速工具：點一下會**覆蓋**全部品項的分攤（適合先設好
  多數品項的分攤，再回頭調整例外的幾項）
- 存進帳本之後每個品項都是獨立的一筆記錄，公帳列表上每筆消費下面會直接列出
  「👥 分攤給誰」（幾個人就顯示幾個人的名字，全員均分會顯示「全員均分」），
  點這個標籤或點金額都能進編輯畫面調整分攤名單，不會動到其他品項
...
提前結清部分或全部欠款。填付款人、收款人、金額、日期跟備註即可，**金額可以選
用主要幣別或外幣輸入**（跟新增記帳一樣有幣別切換鈕）——如果是在日本當地直接
付日圓現金，就切成日圓填實際付的金額，系統會自動用當下的匯率換算。
