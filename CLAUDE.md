...
現在的 prompt 改成優先要求 Gemini 判斷折扣是不是「緊跟在某商品後面、明顯針對
該商品」，是的話直接淨入該商品的 amount／unit_price；只有真正整單性質、對不上
特定商品的折扣才保留成獨立 item（name 加註「折扣：」前綴）。這個判斷交給
Gemini 的視覺理解去做，不是靠關鍵字比對——沒辦法在這個環境用真的 API Key 驗證，
使用者回報有問題時先看是不是這個判斷沒抓對。
